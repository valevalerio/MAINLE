...
from mainle.parsers.breastcancer import BreastcancerParser, breastcancer_system_instructions
from mainle.explainers.breastcancer import BreastcancerExplainer
...
def main(save_history: bool = True, run_critic_agent: bool = True):
    mainleParser = BreastcancerParser(Gpt(model="gpt-4o-mini", system_prompt=breastcancer_system_instructions()))
    user_info = mainleParser.execute()
    if save_history:
        mainleParser.save_history("1_parser.json", include_system_prompt=True)

    mainleExplainer = BreastcancerExplainer()
