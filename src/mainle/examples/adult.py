...
from mainle.parsers.adult import AdultParser, adult_system_instructions
from mainle.explainers.adult import AdultExplainer
...
def main(save_history: bool = True, run_critic_agent: bool = True):
    mainleParser = AdultParser(Gpt(model="gpt-4o-mini", system_prompt=adult_system_instructions()))
    user_info = mainleParser.execute()
    if save_history:
        mainleParser.save_history("1_parser.json", include_system_prompt=True)

    mainleExplainer = AdultExplainer()
