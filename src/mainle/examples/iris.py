...
from mainle.parsers.iris import IrisParser, iris_system_instructions
from mainle.explainers.iris import IrisExplainer
...
from mainle.chat.engines.llama import Llama


def main(save_history: bool = True, run_critic_agent: bool = True):
    mainleParser = IrisParser(Llama(model="qwen3.5:latest", system_prompt=iris_system_instructions()))
    user_info = mainleParser.execute()
    if save_history:
        mainleParser.save_history("1_parser.json", include_system_prompt=True)

    mainleExplainer = IrisExplainer()
    translator_prompt = mainleExplainer.execute(user_info)

    mainleTranslator = LlmTranslator(Llama(model="qwen3.5:latest"))
    ...
    mainleSimplifier = LlmSimplifier(Llama(model="qwen3.5:latest", system_prompt=simplifier_system_instructions()))
    ...
    if run_critic_agent:
        mainleCritic = LlmCritic(Llama(model="qwen3.5:latest", system_prompt=critic_system_instructions()))
