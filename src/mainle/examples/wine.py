...
from mainle.parsers.wine import WineParser, wine_system_instructions
from mainle.explainers.wine import WineExplainer
...
def main(save_history: bool = True, run_critic_agent: bool = True):
    mainleParser = WineParser(Gpt(model="gpt-4o-mini", system_prompt=wine_system_instructions()))
    user_info = mainleParser.execute()
    if save_history:
        mainleParser.save_history("1_parser.json", include_system_prompt=True)

    mainleExplainer = WineExplainer()
