...
# TODO: add support to DeepSeek token count
def count_tokens(prompt: str, model_name: str):
    return 0


class DeepSeekResponse(chat.Response):
...
class DeepSeek(chat.ChatEngine):
    def __init__(self, model, api_key=None, system_prompt="You are a helpful assistant.", tools=None):
        ...
        logging.info(f"Initialized DeepSeek model: {model}")
