...
class BreastcancerExplainer(LoreExplainer):
    def __init__(self):
        self.dataset = pd.read_csv('tabular_datasets/breast-cancer-wisconsin.csv')
        self.dataset.drop(columns = ['Sample_code_number'], inplace = True)
        self.dataset.replace('?', pd.NA, inplace = True)
        self.dataset.dropna(inplace = True)
        self.dataset["class"] = self.dataset["class"].replace({2: "benign", 4: "malignant"})
